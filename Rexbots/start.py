...
import shutil
import pyrogram
import requests  # Added for API-based photo fetching
...
# --- Dynamic Assets (Rotational) - Replaced hardcoded PICS with API fetching logic in send_start ---
# Removed PICS list; now fetching via API for dynamic photos (using waifu.pics and nekos.life for anime/cosplay theme)
...
async def send_start(client: Client, message: Message):
    ...
    # Fetch dynamic photo via API (randomly choose between waifu.pics and nekos.life)
...
async def settings_panel(client, callback_query):
    ...
    # Updated button layout: Single-column for settings to match new streamlined design
...
async def button_callbacks(client: Client, callback_query: CallbackQuery):
   ...
   # --- DEVELOPER INFO ---
    ...
    # --- HOME / START MENU ---
        ...
        bot = await client.get_me()
        # Fetch dynamic photo via API for edit as well
        apis = ["https://api.waifu.pics/sfw/waifu", "https://nekos.life/api/v2/img/waifu"]
        api_url = random.choice(apis)
        try:
            response = requests.get(api_url)
